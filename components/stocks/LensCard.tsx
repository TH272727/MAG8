...
import type { LensRow } from "@/lib/db";
import { LENS_META, type LensSkill, type MetricValue, type Verdict } from "@/lib/schemas";

const ACCENT: Record<LensSkill, { border: string; text: string }> = {
  "stock-scanner": { border: "border-t-fundamentals/70", text: "text-fundamentals" },
  "gt-predictor": { border: "border-t-macro/70", text: "text-macro" },
  "institutional-forecast": { border: "border-t-consensus/70", text: "text-consensus" },
};
...
function metricRows(skill: LensSkill, km: Record<string, MetricValue>): [string, string][] {
  switch (skill) {
    case "stock-scanner":
      return [
...
export default function LensCard({ row }: { row: LensRow }) {
  const meta = LENS_META[row.skill];
  const accent = ACCENT[row.skill];
    ...
    return (
      <section className="panel border-t-2 border-t-danger/60 p-5" aria-label={`${meta.label} lens — errored`}>
        <div className="flex items-baseline justify-between gap-3">
          ...
          <span className="font-mono text-[11px] text-dim">{row.skill}</span>
        </div>
