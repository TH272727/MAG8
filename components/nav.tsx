    ...
    <header className="sticky top-0 z-40 border-b border-hairline bg-void/90 backdrop-blur-sm">
      <nav className="mx-auto flex h-14 max-w-6xl items-center gap-3 px-4 sm:gap-6 sm:px-6">
        ...
        <div className="ml-auto flex min-w-0 items-center gap-0.5 overflow-x-auto sm:gap-2">
          ...
          {!launch && (
            <Link href="/rotation" className="shrink-0 rounded px-2 py-1.5 text-[13px] text-muted transition-colors hover:text-ink sm:px-2.5 sm:text-sm">
              Rotation
            </Link>
          )}
          {!launch && (
          ...
          {!launch && (
            <Link href="/risk" className="shrink-0 rounded px-2 py-1.5 text-[13px] text-muted transition-colors hover:text-ink sm:px-2.5 sm:text-sm">
              Risk
            </Link>
          )}
          {!launch && (
